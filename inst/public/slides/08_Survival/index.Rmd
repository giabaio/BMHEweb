---
title: "&#8291;8. Survival analysis in HTA"
...
output:
  xaringan::moon_reader:
    ...
    nature:
      ...
      highlightLines: yes
      # This is needed to higlight single words in a chunk!
      highlightSpans: true
...
- Relevant quantites are: the .orange[density] and .red[cumulative distribution] functions
   ...
   $$\class{olive}{h(t)=\lim_{\Delta\rightarrow 0} \frac{\Pr(t\leq T \leq t+\Delta\mid T>t)}{\Delta}} \qquad{ `r sftext("and")` } \qquad \class{spanish-red}{H(t)=\int_0^t}\class{olive}{h(u)}\class{spanish-red}{du}$$
...
`r include_fig("censoring.png",width="85%")`
...
`r include_fig("survHEschema.png",width="75%")`

`r icon::fontawesome("github")` [https://github.com/giabaio/survHE](https://github.com/giabaio/survHE)
.alignright[`r icon::fontawesome("firefox")` [`survHE` webpage](https://gianluca.statistica.it/software/survhe/)]
...
```{r comment=NA,warning=FALSE,error=FALSE,message=FALSE,echo=TRUE,options=list(width=2000)}
...
.pull-left[
```{r comment=NA,warning=FALSE,error=FALSE,message=FALSE,echo=TRUE,eval=TRUE,size="tiny"}
# Prints the model estimates for model 1 (Exponential)
print(m.int,mod=1)
```
]
.pull-right[
```{r comment=NA,warning=FALSE,error=FALSE,message=FALSE,echo=TRUE,size="tiny"}
...
.panel-name[Output]
.pull-left[
```{r comment=NA,warning=FALSE,error=FALSE,message=FALSE,fig.width=7,fig.height=7,opts=list(width='52%'),fig.align='center',cache=TRUE,size="tiny"}
# Shows the parameter estimates in the formatting of 'survHE'...
print(m.hmc,digits=4)
```
]
.pull-right[
```{r comment=NA,warning=FALSE,error=FALSE,message=FALSE,fig.width=7,fig.height=7,opts=list(width='50%'),fig.align='center',cache=TRUE,size="tiny"}
...
.small[.alignright[`r icon::fontawesome("arrow-circle-right")` [Next lecture](../09_MM/index.html)]]
